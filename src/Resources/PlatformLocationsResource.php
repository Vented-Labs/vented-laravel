...
use Vented\Data\PlatformLocationData;
use Vented\Results\CollectionResult;
use Vented\Vented;

final readonly class PlatformLocationsResource
{
     ...
     *
     * Operation: platform-locations.index
     *
     * @param  array<string, mixed>  $query
     * @return CollectionResult<PlatformLocationData>
     */
    public function list(array $query = []): CollectionResult
    {
        $operation = $this->client->operation('GET', '/platform-locations')
            ->withQuery($query);

        return $operation->collection(static fn (array $resource): PlatformLocationData => PlatformLocationData::fromArray(self::attributes($resource, true)));
    }
